...
class Achievements:
    ...
    def __init__(self, characters, 
        ...
        if rebuild:
            #预读取成就列表并修正格式
            self._achievements_text = self.__preprocess(raw_achievements_filename)
        else:
            #读取修正完格式的成就列表
    ...
    def write_achievements(self, filename):
        ...
        with open(filename,'wb') as file:
            for x in self._achievements:
                csv.writer(file, delimiter='#').writerow([x.name, x.condition, x.reward])
    ...
    def characters_should_use(self):
        #滤选报酬为武将的成就
        achievements = self.filter(lambda x:isinstance(x.reward_node, Character))
        #滤选内容为80胜的成就
